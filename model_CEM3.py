...
def adversarial_Net(input_im2):
    with tf.variable_scope('adversarial', reuse=tf.AUTO_REUSE):
        out = CEMNet3.adversarial2(input_im2)
    return out
class lowlight_enhance(object):
    def __init__(self, sess):
        ...
        output,out_color,out_edge,mask_color,mask_edge = CEMNet3.net_three_fusion(self.input_low)#输入低光图像
    ...
    def test(self, test_low_data, test_high_data, test_low_data_names, test_high_data_names,save_dir, decom_flag):
        ...
        load_model_status_Decom, _ = self.load(self.saver_Decom, './save_model/0005_01_001/')
        ...
        for idx in range(len(test_low_data)):
            ...
            save_images(os.path.join(save_dir, name + "_out_." + suffix), output)
